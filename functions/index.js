...
app.post('/api/booking-addon/checkout', async (req, res) => {
    ...
    try {
        ...
        logger.info(`[dLocal Go Addon] Creando pago de Addon para negocio ${businessId}`);
        ...
        const errorStatus = err.response?.status || 500;
        const errorData = err.response?.data || err.message;
        logger.error(`[dLocal Go Addon] Error ${errorStatus}:`, errorData);
    ...
    try {
        ...
        if (expectedToken) {
            if (!authHeader || authHeader !== `Bearer ${expectedToken}`) {
                logger.warn(`[Webhook dLocal Addon] Token inválido para negocio ${businessId}`);
                return res.status(401).json({ success: false, message: 'Firma de Webhook no válida' });
            }
        }

        logger.info(`[Webhook dLocal Addon] Recibido para negocio ${businessId}, status: ${status}`);
        ...
        if (status === 'PAID' && businessId) {
            ...
            logger.info(`[Webhook dLocal Addon] Addon de reservas activado para negocio ${businessId}`);
        }

        res.status(200).send('OK');
    } catch (error) {
        logger.error('[Webhook dLocal Addon] Error:', error);
